        ...
        <div className="min-h-screen bg-[#f6eadf] text-gray-800 py-7">
            <div className="max-w-[1200px] mx-auto px-5">
                ...
                <div className={`${eventModalData ? 'flex' : 'hidden'} fixed inset-0 z-50 items-center justify-center bg-black/60 backdrop-blur-sm p-6`} onClick={(e) => { if (e.target === e.currentTarget) closeEventModal() }}>
                    {eventModalData && (
                        <div className="max-w-[920px] w-full max-h-[90vh] overflow-y-auto bg-gradient-to-br from-white to-amber-50 rounded-2xl p-6 shadow-2xl relative border-2 border-amber-300">
                            <button className="sticky top-0 right-0 float-right text-gray-600 text-3xl hover:text-amber-700 transition z-10" onClick={closeEventModal} aria-label="Đóng">&times;</button>
                            
                            <div className="mb-6">
                                <h2 className="text-2xl font-bold text-amber-800 mb-2">{eventModalData.name}</h2>
                                <div className="text-amber-600 font-semibold mb-4">📅 Năm: {eventModalData.year}</div>
                            </div>

                            {/* Tóm tắt */}
                            <div className="mb-6 p-4 bg-amber-100 rounded-lg border border-amber-200">
                                <h3 className="text-lg font-semibold text-amber-800 mb-2">📌 Tóm tắt</h3>
                            ...
                            {eventModalData.context && (
                                <div className="mb-6">
                                    ...
                                    <p className="text-gray-700 leading-relaxed bg-white p-4 rounded-lg border border-amber-100">
                                        {eventModalData.context}
                                    </p>
                                </div>
                            )}
                                ...
                                <div className="mb-6">
                                    ...
                                    <ul className="space-y-3">
                                        {eventModalData.dienbien.map((step, idx) => (
                                            <li key={idx} className="flex gap-3 bg-white p-4 rounded-lg border border-amber-100 hover:bg-amber-50 transition">
                                ...
                                <div className="mb-6">
                                    ...
                                    <ul className="space-y-2">
                                        {eventModalData.ynghia.map((meaning, idx) => (
                                            <li key={idx} className="flex gap-3 bg-gradient-to-r from-amber-50 to-white p-4 rounded-lg border border-amber-100">
